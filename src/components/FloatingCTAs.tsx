    ...
    >
      {ctaItems.map((item, index) => (
        <motion.a
          ...
          whileTap={{ scale: 0.95 }}
          className={`group relative flex items-center justify-center w-12 h-12 rounded-full ${item.bgClass} ${item.hoverClass} shadow-lg transition-all duration-300`}
          aria-label={item.label}
        >
          <item.icon className="w-5 h-5 text-primary-foreground" />
          <span className="absolute right-full mr-3 px-3 py-1 rounded-lg bg-card text-foreground text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg border border-border">
            {item.label}
          </span>
        </motion.a>
      ))}
